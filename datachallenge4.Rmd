...
qplot(rater1,rater2,data=rc)+
  geom_jitter()+
  geom_title('How well did the two raters agree on skin tone?')
```
...
Of those with data, most were considered to have dark skin by the raters (skin tone <= 0.25)
...
All players who received more than 6 red cards were considered darker than average. 
...
rcm %>%
  ...
  summarize(total_reds_given = sum(redCards),
            total_reds_dark = sum(redCards[mean_rating<= 0.25]),
            total_reds_light = sum(redCards[mean_rating>= 0.75]))->
  ref_summary
...
Of those who do give out red cards, most give one red card to a player whow as rated to have darker skin and none to any players rated to have light skin. 
